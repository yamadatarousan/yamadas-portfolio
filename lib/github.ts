  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        console.warn(`GitHub user '${username}' not found or has no public repositories`)
      } else {
    ...
    if (getAllPages && repositories.length === per_page) {
      ...
      while (hasMore) {
        try {
          ...
          const nextRepositories: GitHubRepository[] = await nextResponse.json()
...
// 複数のリポジトリの言語統計を集計
export async function getLanguageStats(repositories: GitHubRepository[]): Promise<Record<string, number>> {
  const languageStats: Record<string, number> = {}
  
  for (const repo of repositories.slice(0, 20)) { // 最大20個のリポジトリから集計
    const languages = await getRepositoryLanguages(repo.full_name.split('/')[0], repo.name)
